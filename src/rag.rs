...
impl RagSystem {
    ...
    pub fn build_local_model(&mut self) -> Result<()> {
        ...
        for entry in WalkDir::new(&self.data_dir) {
            ...
            if path.is_file() {
                ...
                match path.extension().and_then(|s| s.to_str()) {
                    ...
                    Some("txt") | Some("md") => {
                        colour_print(&format!("\t Processing text file: {}", path.display()), "yellow");
        ...
        colour_print(&format!("\t Successfully built local model with {} documents and {} chunks", 
                             self.documents.len(), self.chunks.len()), "green");
    ...
    fn build_word_index(&mut self) {
        for (chunk_idx, chunk) in self.chunks.iter().enumerate() {
            ...
            let words: Vec<String> = chunk.content
            ...
            for word in words {
                self.word_index.entry(word).or_insert_with(Vec::new).push(chunk_idx);
            }
        }
    }
    ...
    pub fn search_local(&self, query: &str, top_k: usize) -> Vec<(f32, &DocumentChunk)> {
        ...
        for word in &query_words {
            if let Some(chunk_indices) = self.word_index.get(word) {
                ...
                let inverse_document_frequency = ((self.chunks.len() as f32) / document_frequency).ln();
                
                for &chunk_idx in chunk_indices {
                    if let Some(chunk) = self.chunks.get(chunk_idx) {
                        // Term frequency: how often the word appears in this chunk
                        let term_frequency = chunk.content.to_lowercase().matches(word).count() as f32;
        ...
        let mut results: Vec<(f32, &DocumentChunk)> = chunk_scores
            ...
            .filter_map(|(idx, score)| {
                self.chunks.get(idx).map(|chunk| (score, chunk))
            })
